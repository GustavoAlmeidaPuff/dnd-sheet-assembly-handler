...
export function CharacterCreationProvider({ children }: { children: React.ReactNode }) {
  ...
  const setRace = useCallback((race: Race | null) => {
    if (!race) {
    ...
    const abilityBonuses: { [key: string]: number } = {};
    race.ability_bonuses.forEach((bonus) => {
      // A API pode retornar "str", "strength", "str-score", etc.
      let abilityKey = bonus.ability_score.index.toLowerCase();
      // Remover sufixos como "-score"
    ...
    // Se já temos atributos, aplicar bônus raciais
    if (character.attributes) {
      const baseAttributes = { ...character.attributes };
      // Remover bônus anteriores se houver
      const updatedAttributes = applyRacialBonuses(baseAttributes, abilityBonuses);
  ...
  const setAttributes = useCallback((attributes: Character['attributes']) => {
    // Se temos uma raça selecionada, aplicar bônus raciais
    if (character.race) {
